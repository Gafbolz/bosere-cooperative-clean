...
async def get_all_loans(
    ...
    for l in loans:
        ...
        loan_list.append({
            ...
            "monthly_payment": l.monthly_payment,
            "total_repayment": l.total_repayment,
            "remaining_balance": max(0,float(l.total_repayment or 0) - float(sum(float(r.amount or 0) for r in l.repayments))),
            "purpose": l.purpose,
            "status": get_status_value(l.status),
            "created_at": l.created_at.isoformat(),
            "approved_at": l.approved_at.isoformat() if l.approved_at else None,
            "total_repaid": float(sum(float(r.amount or 0) for r in l.repayments)),
